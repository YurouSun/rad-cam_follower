...
class Car2LineFollowerSafeNode(Node):
    def __init__(self):
        super().__init__('car2_line_follower_safe_node')
        self.get_logger().info('🚗 Car 2 SAFE 副本节点 [Y轴正反馈修复版：打断死亡螺旋 + 强化防撞墙 + 20Hz动态速度平滑]')
    ...
    def cb_radar_pcl(self, msg):
        ...
        try:
            for p in point_cloud2.read_points(msg, field_names=('x', 'y', 'z'), skip_nans=True):
                ...
                if abs(raw_x) > 1.5: continue 
                valid_points.append((d, raw_x, raw_y))
        except Exception: return
            
        if len(valid_points) < cluster_min_pts:
            self.get_logger().warn(
                f"Radar valid_points too few: {len(valid_points)} < {cluster_min_pts}",
        ...
        if not clusters:
            self.get_logger().warn(
                f"Radar no clusters: valid_points={len(valid_points)}, eps={cluster_eps}, min_pts={cluster_min_pts}",
                throttle_duration_sec=1.0
            )
            return
        ...
        if best_candidate is None:
            self.get_logger().warn(
                f"Radar no best candidate: clusters={len(clusters)}, has_history={has_history}, max_jump={max_jump}",
                throttle_duration_sec=1.0
            )
            return
        ...
        if best_candidate is not None:
            ...
            self.get_logger().info(
                f"🎯 target accepted: front={self.z_pcl[0]:.2f}m, right={self.z_pcl[1]:.2f}m, valid={len(valid_points)}, clusters={len(clusters)}",

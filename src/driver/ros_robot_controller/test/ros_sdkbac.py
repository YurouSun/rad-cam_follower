...
class Board:
    ...
    def __init__(self, device="/dev/ttyACM0", baudrate=115200, timeout=10):
    ...
    def set_motor_speed(self, speeds):
        ...
        # Format: 01 03 ID1 Speed1 ID2 Speed2 ID3 Speed3
        # 0x01: Subcommand, 0x03: Count
        data = [0x01, 0x03]
        data.append(0x01) # ID 1 (X)
        data.extend(struct.pack("<f", vx))
        data.append(0x02) # ID 2 (Y)
        data.extend(struct.pack("<f", vy))
        data.append(0x03) # ID 3 (Yaw/Rotation)
        data.extend(struct.pack("<f", vz))
        
        self.buf_write(PacketFunction.PACKET_FUNC_MOTOR, data)
        # print(f"DEBUG: Motor Data Sent: {[hex(x) for x in data]}")

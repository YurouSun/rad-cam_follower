...
# 定义输出目录名（时间戳格式），允许外部覆盖
if [ -z "${BAG_DIRECTORY_NAME}" ]; then
...
if [ -n "${DEFER_DRIVER_CMD}" ]; then
	...
	if [ "${STOP_AFTER_DRIVER}" = "1" ]; then
		echo ">>> STOP_AFTER_DRIVER=1，驱动命令结束后将自动停止录制。"
	fi
